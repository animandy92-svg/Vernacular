...
export function Pronunciation({ entry, compact = false }: { entry: WordEntry; compact?: boolean }) {
  ...
  const play = () => {
    if (!('speechSynthesis' in window)) { setMessage('Audio is unavailable. Use the pronunciation guide below.'); return }
    try {
      ...
      utterance.rate = 0.72
      utterance.onerror = (event) => { if (event.error !== 'interrupted' && event.error !== 'canceled') setMessage('Audio is unavailable on this device. Follow the written guide.') }
      setMessage(voice ? 'Playing device pronunciation.' : 'Your device voice may approximate this language. Follow the written guide.')
      window.speechSynthesis.speak(utterance)
    } catch { setMessage('Audio is unavailable on this device. Follow the written guide.') }
  }
