...
export function Onboarding({ onComplete, initialProfile }: Props) {
      ...
      <section className="onboarding-panel">
        ...
        {step === 0 && (
          <div className="onboarding-step">
            ...
            <input autoFocus id="learner-name" className="text-field" value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && canContinue && next()} placeholder="e.g. Amanda" maxLength={30} />
          </div>
        )}
        ...
        {step === 1 && (
          <div className="onboarding-step">
            ...
            <input autoFocus id="local-name" className="text-field" value={localName} onChange={(event) => setLocalName(event.target.value)} placeholder="e.g. Adwoa" maxLength={30} />
            <span className="field-label field-label--compact">Language to explore</span>
            <div className="language-pill-row">
              {LANGUAGES.map((item) => <button className={language === item.code ? 'selected' : ''} key={item.code} onClick={() => setLanguage(item.code)}><span style={{ backgroundColor: item.color }}>{item.name.slice(0, 2)}</span><strong>{item.name}</strong>{language === item.code && <Check size={16} />}</button>)}
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="onboarding-step companion-builder-step">
            ...
            <div className="companion-builder">
              <div className="companion-preview"><Companion character={companion} pose="talk" item="none" size={190} /><span>“Ready when you are!”</span></div>
              <div className="companion-controls">
                ...
                <div className="avatar-choice-grid">{AVATARS.map((avatar) => <button className={companion.avatar === avatar.id ? 'selected' : ''} key={avatar.id} onClick={() => chooseAvatar(avatar.id, avatar.label)}><img src={`/characters/${avatar.id}.png`} alt="" /><span><strong>{avatar.label}</strong><small>{avatar.detail}</small></span>{companion.avatar === avatar.id && <Check size={16} />}</button>)}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="onboarding-step meet-step">
            ...
            <div className="level-grid level-grid--compact">{LEVELS.map((item) => <button className={`level-choice ${level === item.id ? 'selected' : ''}`} key={item.id} onClick={() => setLevel(item.id)}><strong>{item.label}</strong><small>{item.description}</small></button>)}</div>
            <div className="goal-picker"><div><span className="goal-icon"><Flame size={20} /></span><span><strong>Daily goal</strong><small>puzzles per day</small></span></div><div className="segmented-control">{[3, 5, 10].map((goal) => <button className={dailyGoal === goal ? 'selected' : ''} key={goal} onClick={() => setDailyGoal(goal)}>{goal}</button>)}</div></div>
          </div>
        )}

        <div className="onboarding-actions">
          {step > 0 && <button className="button button--ghost" onClick={() => setStep((current) => current - 1)}><ArrowLeft size={18} /> Back</button>}
          <button className="button button--primary" disabled={!canContinue} onClick={next}>{step === 3 ? <>Start exploring <Sparkles size={18} /></> : <>Continue <ArrowRight size={18} /></>}</button>
        </div>
      </section>
    </main>
  )
